...
Edit `.env` to set `TELEGRAM_BOT_TOKEN`, `GEMINI_API_KEY`, and `LLM_ORDER` / `LLM_ORDER_FAST`. Drop `--telegram` for CLI mode, or add `--dev` to use `TELEGRAM_BOT_TOKEN_DEV`.
...
- **`config/user_config.json`** — admin and allowed users with their accessible agents. Users not listed in any group are silently ignored (no reply). See [example](config/user_config.example.json).
