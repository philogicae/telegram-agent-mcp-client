...
class TestFixedTelegram:
    ...
    def test_alphabetic_tag_like_text_passes_through(self):
        # The escaper only escapes '<' that cannot start an HTML tag; the
        # result is still sanitized server-side by Telegram's parser.
        assert "<notatag>" in fixed_telegram(None, "a <notatag> b")
