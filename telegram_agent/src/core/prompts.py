...
# Meta-procedures:
- `find_media` for requests looking like "find <movie-or-series>" : `prepare_search_query` -> `search_torrents` -> If results are homogeneous (all the same item) always auto-pick the best option according to `search_torrents` priority rules (is 1080p > is x265 > max seeders+leechers > smaller file size) -> `get_magnet_link` -> `download_torrent` -> `get_torrent_stats` -> Reply with torrent info and status.
