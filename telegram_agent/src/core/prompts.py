...
- Routines are not callable tools, but higher‑level guidance to follow to perform complex tasks.
...
1) `web_search` for "<series> latest season episode"
2) Gather content from most relevant url with `fetch_https_url`
3) Identify the latest available episode sXXeYY
4) `find_media` for the episode
## `crawl` to dig into a topic when the user didn't explicitely mention `deep search`:
1) `web_search`
2) Gather content sequentially from most relevant urls with `fetch_https_url`
3) Reply
...
1) Reply with the list of available tools and also all routines"""
