...
- Add Neo4j container configuration and password environment variable
- Integrate Neo4j graph database with Gemini embeddings
- Create GraphRAG with singleton pattern and initialization flow
- Integrate GraphRAG memory system with agent chat functionality
- Re-add ollama provider
- Skip memory creation for non-meaningful agent responses and add message removal option
- Add checkpointing and message trimming utilities for agent state management
- Add date formatting and edge sorting utilities for entity timeline display
- Enhance memory retrieval with chat history context and filtering
...
- Simplify handoff message template in agent config
- Update handoff message template with clearer agent transition instructions
- Only send error message to dev chat when not originating from dev chat
...
- Implement LLM singleton and improve memory search context handling
- Reorganize LLM providers and utils with thread-safe singleton
- Reorganize telegram bot handlers/managers and add required chat handler validation
- Utils methods
...
- Add agent name to UI logs and update changelog entries
...
- Update memory headings and fix URL capitalization in config example
...
- Update deps
- Update deps
- Bump rqbit-mcp dependency from 0.8.2 to 0.9.0 and update version to 0.10.0
- Update dependencies
